...
def call_ollama_text(prompt_text):
    """Single-shot text generation via Ollama (no images, no system message).
    Mirrors the original's Gemini CLI stdin piping: the entire prompt_text
    is sent as a single user message, identical context to what Gemini CLI received."""
    import ollama  # type: ignore
    response = ollama.chat(model=OLLAMA_MODEL, messages=[
        {'role': 'user', 'content': prompt_text}
    ])
    msg = response.get('message', getattr(response, 'message', {}))
...
def evaluate_chart_with_history(taxonomy_text, strategy_text, code_text, image_paths, claim_a, claim_b, log_path="evaluator_log.txt"):
    """3-turn evaluator chat via Ollama — identical to the original orchestrator."""
    import ollama  # type: ignore
    import re
...
def main():
    ...
    # (Identical context string to original — single blob piped to LLM)
    ...
    for strategy_revision in range(MAX_STRATEGY_REVISIONS):
        ...
        for strategy_attempt in range(MAX_STRATEGY_RETRIES):
            print(f"\n🧠 STEP 1: Consulting Ollama ({OLLAMA_MODEL}) for idea generation (Revision {strategy_revision + 1}/{MAX_STRATEGY_REVISIONS}, Attempt {strategy_attempt + 1}/{MAX_STRATEGY_RETRIES})...")
            
            # call_ollama_text sends idea_history as a single user message,
            # exactly mirroring the original's stdin pipe to Gemini CLI
            content = call_ollama_text(idea_history)
        ...
        # Natively strip any "chain of thought" or conversational preamble
        ...
        for code_revision in range(MAX_CODE_REVISIONS):
            ...
            for code_attempt in range(MAX_CODE_RETRIES):
                print(f"\n🧠 STEP 2: Consulting Ollama ({OLLAMA_MODEL}) for Code Generation (Revision {code_revision + 1}/{MAX_CODE_REVISIONS}, Attempt {code_attempt + 1}/{MAX_CODE_RETRIES})...")
            
                # Build the exact same merged prompt the original sends to Gemini CLI
                ...
                if code_history:
                    ollama_code_input = f"{merged_text}\n\n{code_history}"
                else:
                    ollama_code_input = merged_text
    
                # call_ollama_text sends the entire merged prompt as a single user message
                generated_code = call_ollama_text(ollama_code_input)
